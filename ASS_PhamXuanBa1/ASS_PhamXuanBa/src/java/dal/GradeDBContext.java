...
"where cid=? and st.sid = ?";
    ...
    public Double getEverageBySidAndSuid(int sid, int suid) {
        String sql = "select SUM(g.score * asm.weight)  AS average_score\n" +
"from grades g join students s on g.sid =s.sid\n" +
"join exams e on e.eid = g.eid\n" +
"join assesments asm on asm.aid = e.aid\n" +
"join subjects su on su.subid = asm.subid\n" +
"where s.sid =? and su.subid = ?\n" +
"group by \n" +
"	s.sid, s.sname, su.subname";
